...
abstract class tablePrt {
    ...
    static public function getInstance($table = '') {
        ...
        if(!isset($instances[$table])) {
            $class = 'table'. strFirstUp($table). strFirstUp(PRT_CODE);
            ...
            if($id) {
                ...
                foreach($data as $field) {
                    ...
                    foreach($field as $k => $v) {
                        if(isset($this->_fields[$k])) {
                            $row[$k] = toeCreateObjPrt('fieldPrt', array(
                                    $this->_fields[$k]->name,
                                    $this->_fields[$k]->html,
                                    $this->_fields[$k]->type,
    ...
    public function getTable($transform = false) {
        if($transform)
            return dbPrt::prepareQuery($this->_table);
        else
            return $this->_table;
    }
    ...
    public function get($fields = '*', $where = '', $tables = '', $return = 'all') {
        ...
        return dbPrt::get($query, $return);
    }
    public function store($data, $method = 'INSERT', $where = '') {
        $method = strtoupper($method);
		if($this->_escape) {
			$data = dbPrt::escape($data);
		}
        $query = '';
        switch($method) {
            case 'INSERT':
        ...
        if(empty($fields)) {
            $this->_addError(langPrt::_('Nothig to update'));
            return false;
        }
        ...
        if(dbPrt::query($query)) {
            if($method == 'INSERT')
                return dbPrt::lastID();
            else
                return true;
        } else
			$this->_addError(langPrt::_('Database error. Please contact your developer.'));
        return false;
    }
            ...
            foreach($data as $k => $v) {
                if(array_key_exists($k, $this->_fields) || $k == $this->_id) {
                    ...
                    if($this->_fields[$k]->adapt['dbTo']) 
                        $val = fieldAdapterPrt::_($val, $this->_fields[$k]->adapt['dbTo'], fieldAdapterPrt::DB);
                    if($validate) {
                        if (is_object($this->_fields[$k])) {
                            $objForValidation = clone $this->_fields[$k];
                            $objForValidation->setValue($val);
                            if($errors = validatorPrt::_($objForValidation)) {
    ...
    /**
     * Add new fieldPrtPrt for children table (@see class field)
    ...
    protected function _addField($name, $html = 'text', $type = 'other', $default = '', $label = '', $maxlen = 0, $dbAdapt = '', $htmlAdapt = '', $description = '') {
        $this->_fields[$name] = toeCreateObjPrt('fieldPrt', array($name, $html, $type, $default, $label, $maxlen, $dbAdapt, $htmlAdapt, $description));
        return $this;
    }
    ...
    public function exists($value, $field = '') {
        if(!$field)
            $field = $this->_id;
        return dbPrt::get('SELECT '. $this->_id. ' FROM '. $this->_table. ' WHERE '. $field. ' = "'. $value. '"', 'one');
    }
            ...
            switch($f->type) {
                case 'date':
                    if($d[$key] == '0000-00-00' || empty($d[$key]))
                        ...
                        $d[$key] = date(PRT_DATE_FORMAT, dbPrt::dateToTime($d[$key]));
                    }
                    break;
                case 'int':
                case 'tinyint':
    ...
    public function getLastInsertID() {
        return dbPrt::get('SELECT MAX('. $this->_id. ') FROM '. $this->_table, 'one');
    }
