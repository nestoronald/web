...
class langPrt {
	...
	static public function extract($d = array('dir' => '', 'langPrt' => '')) {
		$data = array();
		if(isset($d['dir']) && !empty($d['dir']))
			$langDirPath = $d['dir'];
		else if(isset($d['langPrt']) && !empty($d['langPrt']))
			$langDirPath = PRT_LANG_DIR. $d['langPrt']. DS;
		else
			$langDirPath = PRT_LANG_DIR. PRT_WPLANG. DS;
