<?php
abstract class modulePrt extends baseObjectPrt {
	...
	public function __construct($d, $params = array()) {
		...
		if(isset($d['ex_plug_dir']) && !empty($d['ex_plug_dir'])) {
			...
			$this->setExternalDir( utilsPrt::getExtModDir($d['ex_plug_dir']) );
			$this->setExternalPath( utilsPrt::getExtModPath($d['ex_plug_dir']) );
		}
	}
	...
	public function getModDir() {
		if(empty($this->_externalDir)) {
			return PRT_MODULES_DIR. $this->getCode(). DS;
		} else {
	...
	public function getModPath() {
		if(empty($this->_externalPath)) {
			return PRT_MODULES_PATH. $this->getCode(). '/';
		} else {
	...
	protected function _createController() {
		...
		if(file_exists($this->getModDir(). 'controller.php')) {
			...
			if(import($this->getModDir(). 'controller.php')) {
				$className = toeGetClassNamePrt($this->getCode(). 'Controller');
			}
			if(!empty($className)) {
	...
	protected function _createHelper() {
		...
		if (file_exists($this->getModDir().'helper.php')) {
			...
			importClassPrt($helper, $this->getModDir(). 'helper.php');
	...
	public function __call($name, $arguments) {
		...
		errorsPrt::push(langPrt::_(array('Module', $this->_code, 'method', $name, 'undefined')), errorsPrt::FATAL);
	}
