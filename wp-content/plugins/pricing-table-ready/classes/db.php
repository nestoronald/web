...
class dbPrt {
     ...
     * #__  Wordpress prefix
     * ^__  Store plugin tables prefix (@see PRT_DB_PREF if config.php)
    ...
    static public function prepareQuery($query) {
        ...
        return str_replace(
                array('#__', '^__', '@__'), 
                array($wpdb->prefix, PRT_DB_PREF, $wpdb->prefix. PRT_DB_PREF),
                $query);
    }
    ...
    static public function timeToDate($timestamp = 0) {
        if($timestamp) {
            if(!is_numeric($timestamp))
                $timestamp = dateToTimestampPrt($timestamp);
    ...
    static public function dateToTime($date) {
        ...
        if(strpos($date, PRT_DATE_DL)) return dateToTimestampPrt($date);
        $arr = explode('-', $date);
        return dateToTimestampPrt($arr[2]. PRT_DATE_DL. $arr[1]. PRT_DATE_DL. $arr[0]);
    }
