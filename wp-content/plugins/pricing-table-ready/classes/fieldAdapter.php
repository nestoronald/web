...
class fieldAdapterPrt {
    ...
    static public function _($fieldOrValue, $method, $type) {
        if(method_exists('fieldAdapterPrt', $method)) {
            switch($type) {
                case self::DB:
    ...
    static public function userFieldDestHtml($field) {
        $field->htmlParams['optionsPrt'] = array();
    ...
    static public function userFieldDestToDB($value) {
        return utilsPrt::jsonEncode($value);
    }
    static public function userFieldDestFromDB($value) {
        return utilsPrt::jsonDecode($value);
    }
    static public function taxDataHtml($field) {
        ...
        $categories = framePrt::_()->getModule('products')->getCategories();
        $brands = framePrt::_()->getModule('products')->getBrands();
        $cOptions = array();
        $bOptions = array();
        if(!empty($categories)) {
        ...
        return '<div>'. langPrt::_('Apply To'). '
            <div id="tax_address">
                <b>'. langPrt::_('Address'). '</b><br />
                '. langPrt::_('Destination'). ':'. htmlPrt::checkboxlist('params[dest]', array('optionsPrt' => $listOfDest)). '<br />
                '. langPrt::_('Country'). ':'. htmlPrt::countryList('params[country]', array('notSelected' => true, 'value' => $field->value['country'])). '<br />
            </div>
            <div id="tax_category">
                <b>'. langPrt::_('Categories'). '</b><br />
                '. (empty($cOptions) ? langPrt::_('You have no categories') : htmlPrt::checkboxlist('params[categories][]', array('optionsPrt' => $cOptions))). '<br />
                    <b>'. langPrt::_('Brands'). '</b><br />
                '. (empty($bOptions) ? langPrt::_('You have no brands') : htmlPrt::checkboxlist('params[brands][]', array('optionsPrt' => $bOptions))). '<br />
            </div>
            <div>'. langPrt::_('Tax Rate').': '. htmlPrt::text('params[rate]', array('value' => $field->value['rate'])).'</div>
            <div>'. langPrt::_('Absolute').': '. htmlPrt::checkbox('params[absolute]', array('checked' => $field->value['absolute'])).'</div>
        </div>';
    }
    ...
    static public function getCountries($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if(empty(self::$countries))
                self::$countries = self::getCachedCountries();
            if($notSelected) {
				$options[ $notSelected ][0] = is_bool($notSelected) ? langPrt::_('Not selected') : langPrt::_($notSelected);
			}
    ...
    static public function getStates($notSelected = false) {
        ...
        if(empty($options[ $notSelected ])) {
            ...
            if($notSelected) {
				...
				$options[ $notSelected ][0] = array('name' => langPrt::_( $notSelectedLabel ), 'country_id' => NULL);
			}
    ...
    static public function getExtraFieldOptions($field_id) {
        ...
        $options = framePrt::_()->getModule('optionsPrt')->getHelper()->getOptions($field_id);
        if (!empty($options)) {
    ...
    static public function getFieldAttributes($params){
        ...
        if (!empty($params->attr)) {
            foreach ($params->attr as $key=>$value) {
                $output .= langPrt::_($key).':<br />';
                $output .= htmlPrt::text('params[attr]['.$key.']',array('value'=>$value)).'<br />';
            }
        } else {
                $output .= langPrt::_('class').':<br />';
                $output .= htmlPrt::text('params[attr][class]',array('value'=>'')).'<br />';
                $output .= langPrt::_('id').':<br />';
                $output .= htmlPrt::text('params[attr][id]',array('value'=>'')).'<br />';
        }
        return $output;
    }
    /**
    ...
    static function productFieldCategories($field){
        if(!empty($field->htmlParams['optionsPrt']))
            return;
        ...
        if(!empty($categories)) {
            if(!is_array($field->value['categories']))
                    $field->value['categories'] = array();
            $field->htmlParams['optionsPrt'][0] = in_array(0,$field->value['categories'])?langPrt::_('Deselect All'):langPrt::_('Select All');
            foreach($categories as $c) {
                $field->htmlParams['optionsPrt'][$c->term_taxonomy_id] = $c->cat_name;
            }
        }*/
    }
	...
	static public function getCachedCountries($clearCache = false) {
		if(empty(self::$countries) || $clearCache)
			self::$countries = framePrt::_()->getTable('countries')->getAll('id, name, iso_code_2, iso_code_3');
	...
	static public function getCachedStates($clearCache = false) {
		if(empty(self::$states) || $clearCache)
			self::$states = framePrt::_()->getTable('states')
				->leftJoin( framePrt::_()->getTable('countries'), 'country_id' )
