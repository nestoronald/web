...
class framePrt {
    ...
    public function __construct() {
        $this->_res = toeCreateObjPrt('response', array());
        
    }
    static public function getInstance() {
        static $instance;
        if(!$instance) {
            $instance = new framePrt();
        }
        return $instance;
    }
        ...
        if($activeModules) {
            foreach($activeModules as $m) {
                ...
                if(!empty($m['ex_plug_dir'])) {
                    $moduleLocationDir = utilsPrt::getExtModDir( $m['ex_plug_dir'] );
                }

                if(is_dir($moduleLocationDir. $code)) {
                    $this->_allModules[$m['code']] = 1;
                    if((bool)$m['active']) {
                        importClassPrt($code. strFirstUp(PRT_CODE), $moduleLocationDir. $code. DS. 'mod.php');
                        $moduleClass = toeGetClassNamePrt($code);
    ...
    public function init() {
        ...
        register_activation_hook(  PRT_DIR. DS. PRT_MAIN_FILE, array('utilsPrt', 'activatePlugin')  ); //See classes/install.php file
        register_deactivation_hook(PRT_DIR. DS. PRT_MAIN_FILE, array('utilsPrt', 'deactivatePlugin'));
        
        add_action('admin_notices', array('errorsPrt', 'displayOnAdmin'));
    ...
    public function checkPermissions($code, $action) {
        if($this->havePermissions($code, $action))
            ...
            exit(langPrt::_e('You have no permissions to view this page'));
        }
    }
    /**
            ...
            if(!empty($permissions)) {  // Special permissions
                if(isset($permissions[S_METHODS]) 
                    ...
                    if(array_key_exists($action, $permissions[S_METHODS])) {        // Permission for this method exists
                        $currentUserPosition = framePrt::_()->getModule('user')->getCurrentUserPosition();
                        if((is_array($permissions[ PRT_METHODS ][ $action ]) && !in_array($currentUserPosition, $permissions[ PRT_METHODS ][ $action ]))
                            || (!is_array($permissions[ PRT_METHODS ][ $action ]) && $permissions[S_METHODS][$action] != $currentUserPosition)
                        ) {
                            $res = false;
                        }
                    }
    ...
    protected function _extractTables($tablesDir = PRT_TABLES_DIR) {
    ...
    protected function _extractTable($tableName, $tablesDir = PRT_TABLES_DIR) {
        importClassPrt('noClassNameHere', $tablesDir. $tableName. '.php');
        $this->_tables[$tableName] = tablePrt::_($tableName);
     ...
     * @return object table
     * @example framePrt::_()->getTable('products')->getAll()
     */
    ...
    public function isTplEditor() {
        $tplEditor = reqPrt::getVar('tplEditor');
        return (bool) $tplEditor;
    }
	public function isTablesEditAdminPage() {
		//return reqPrt::getVar('action') == 'edit' ? true : false;
               ...
               if($scriptFile == 'post.php' // Edit post
                       ...
                       && get_post_type($get['post']) == PRT_TYPE_POST // check if this is product
               ) {        
                       return true;
               }
               return false;
