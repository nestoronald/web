...
class validatorPrt {
    ...
    static public function validate($field, $label = '', $validate = array()) {
        self::$errors = array();
        if(is_object($field) && get_class($field) != 'fieldPrt') {
            $value = $field;
            $field = new fieldPrt('noMatter');
            $field->label = $label;
        ...
        if(!empty($field->validate)) {
            foreach($field->validate as $v) {
                 if(method_exists('validatorPrt', $v)) 
                    self::$v($field);
            }
        }
        if(method_exists('validatorPrt', $field->type)) {
            $validate = $field->type;
            self::$validate($field);
        }
        if($field->maxlen) {
    ...
    static public function validLen($field, $label = '', $validate = array()) {
        if( !(bool) (strlen($field->value) <= $field->maxlen)) {
			self::addError(langPrt::_(array('Invalid length for', $field->label, ', max length is', $field->maxlen)), $field->name);
			return false;
		}
		return true;
    }
    ...
    static public function numeric($field) {
        if(!is_numeric($field->value) && !empty($field->value)) {
            self::addError(langPrt::_(array('Invalid numeric value for', $field->label)), $field->name);
            return false;
        }
        return true;
    }
    ...
    static public function notEmpty($field) {
        if(!self::_notEmpty($field->value)) {
            self::addError(langPrt::_(array('Please enter', $field->label)), $field->name);
            return false;
        }
        return true;
    }
    static public function selectNotEmpty($field) {
        if(empty($field->value)) {
            self::addError(langPrt::_(array('Please select', $field->label)), $field->name);
            return false;
        }
        return true;
    }
    static public function email($field) {
        if(!is_email($field->value)) {
            self::addError(langPrt::_(array('Invalid', $field->label)), $field->name);
            return false;
        } elseif(email_exists($field->value)) {
            self::addError(langPrt::_(array($field->label, 'is already registered')), $field->name);
            return false;
        }
        return true;
    }
    ...
    static public function getProductValidationMethods() {
        $res = array();
        $all = get_class_methods('validatorPrt');
        foreach($all as $m) {
