...
class fieldPrt {
	 ...
	 */
	public $adapt = array('htmlPrt' => '', 'dbFrom' => '', 'dbTo' => '');
	/**
	...
	public function setValue($value, $fromDB = false) {
		if(isset($this->adapt['dbFrom']) && $this->adapt['dbFrom'] && $fromDB)
			$value = fieldAdapterPrt::_($value, $this->adapt['dbFrom'], fieldAdapterPrt::DB);
		$this->value = $value;
	}
	...
	public function drawHtml($tag, $id) {
		if(method_exists(html, $this->html)) {
			...
			if($this->adapt['htmlPrt']) {
				fieldAdapterPrt::_($this, $this->adapt['htmlPrt'], fieldAdapterPrt::HTML);
			}
			$params = $this->processParams($tag, $id);
			if ($params != '')
				return $params;
			if ($this->name == 'default_value') {
				$optionsFromDb = framePrt::_()->getModule('optionsPrt')->getHelper()->getOptions($id);
				if (!empty($optionsFromDb)) {
					$options = array(0 => langPrt::_('Select'));
					foreach($optionsFromDb as $k => $v)
						$options[$k] = $v;
					$method = 'selectbox';
					$this->htmlParams['optionsPrt'] = $options;
				}
			}
			$htmlContent = htmlPrt::$method($this->name, $this->htmlParams);
			if(!empty($this->errorEl)) {
				if(is_bool($this->errorEl))
					$errorEl = '<div class="toeErrorForField toe_'. htmlPrt::nameToClassId($this->name). '"></div>';
	...
	public function displayValue() {
		...
		switch($this->html) {
			case 'countryList':
				$value = fieldAdapterPrt::displayCountry($this->value);
				break;
			case 'statesList':
				$value = fieldAdapterPrt::displayState($this->value);
				break;
			case 'checkboxlist':
				$options = $this->getHtmlParam('optionsPrt');
				$value = array();
	...
	public function viewField($name, $fieldValue = '') {
		$method = $this->html;
		$options = framePrt::_()->getModule('optionsPrt')->getHelper()->getOptions($this->id);
		$attrs = '';
		...
		$params = array('optionsPrt'=>$options, 'attrs' => $attrs, 'value' => $fieldValue, 'checked' => $checked);
		$output = '';
		if(method_exists('htmlPrt', $method)) {
			$output .= htmlPrt::$method($name, $params);
			$output .= htmlPrt::hidden('extra_field['.$this->name.']',array('value'=>$this->id));
		}
		return $output;
	}
	...
	public function processParams($tag, $id){
		...
		if ($this->name == "params") {
			...
			switch ($tag) {
				case 5: 
					$add_option = langPrt::_('Add Checkbox');
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 9: 
					$add_option = langPrt::_('Add Item');
					$options_tag = '';
					$image_tag = ' style="display:none"';
				break;
				case 12:
				...
				case 10:
					$options_tag = '';
					$add_option = langPrt::_('Add Radio Button');
					$image_tag = ' style="display:none"';
				break;
				case 8:
					$image_tag = '';
			...
			if ($tag > 0 || $id == 0) {
				$output .= '<div class="options options_tag"'.$options_tag.'>';
					...
					$output .= fieldAdapterPrt::_($id,'getExtraFieldOptions',fieldAdapterPrt::STR);
				$output .= '</div>';

				$output .= '<div class="options image_tag"'.$image_tag.'>'.langPrt::_('Dimensions').':<br />';
					$params->width?$width = $params->width:'';
					$params->height?$height = $params->height:'';
					$output .= langPrt::_('width').':<br />';
					$output .= htmlPrt::text('params[width]',array('value'=>$width)).'<br />';
					$output .= langPrt::_('height').':<br />';
					$output .= htmlPrt::text('params[height]',array('value'=>$height)).'<br />';
				$output .= '</div>';
			}
			if($this->adapt['htmlParams']) {
				$output .= fieldAdapterPrt::_($this, $this->adapt['htmlParams'], fieldAdapterPrt::STR);
			} else {
				$output .= '<a href="javascript:void(0);" class="set_properties">'.langPrt::_('Click to set field "id" and "class"').'</a>';
				$output .= '<div class="attributes" style="display:none;">'.langPrt::_('Attributes').':<br />';
				$output .= fieldAdapterPrt::_($params,'getFieldAttributes',  fieldAdapterPrt::STR);
				$output .= '</div>';
			}
			return $output;
		}
	...
	function checkVarFromParam($param, $element) {
		return utilsPrt::xmlAttrToStr($param, $element);
	  ...
	  // load xml structure of parameters
	   ...
	   foreach ($config->params->param as $param) {
		 // read the variables
		  ...
		  $helper = $this->checkVarFromParam($param,'helperPrt');
		  $module = $this->checkVarFromParam($param,'modulePrt');
	...
	public function drawConfig($module, $additionalOptions = array()) {
		if(!framePrt::_()->getModule($module)) 
			return false; 
		// check for xml file with params structure  
	   if(framePrt::_()->getModule($module)->isExternal())
		   $config_xml = framePrt::_()->getModule($module)->getModDir(). 'mod.xml';
	   else
		   $config_xml = PRT_MODULES_DIR.$module.DS.'mod.xml';

	   if (!file_exists($config_xml)) {
		   // if there is no configuration file for this $module
		   return langPrt::_('There are no configuration options for this module');
	   }
	   $output = '';
		   ...
		   foreach ($params as $param) {
			   ...
			   foreach ($configOptions as $key=>$value){
				  ...
				  // if helper is needed to render the object
					  ...
					  $helper = framePrt::_()->getModule($hmodule)->getHelper();
					  if ($helper) {
				  ...
				  // filling the parameters to build html element
					 $htmlParams = array('value'=>$fieldValue,'optionsPrt'=>$options);
					 if($method == 'checkbox') {
						 $htmlParams['value'] = 1;
				  ...
				  // output label and html element
					 $output .= '<label>'.langPrt::_($configOptions[$key]['label']);
					 if ($configOptions[$key]['description'] != '') {
						 $output .= '<a class="toeOptTip" tip="'.langPrt::_($configOptions[$key]['description']).'"></a>';
					 }
					 $output .= '</label><br />';
					 $output .= htmlPrt::$method($name,$htmlParams).'<br />';
					 $output .= '</div>';
			   }
			   $i++;
			 $output .= '</div>';
