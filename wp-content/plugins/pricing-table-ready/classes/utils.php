...
class utilsPrt {
    ...
    static public function copyDirectories($source, $destination) {
        if(is_dir($source)) {
            ...
            while ( FALSE !== ( $readdirectory = $directory->read() ) ) {
                ...
                if (is_dir($PathDir)) {
                    utilsPrt::copyDirectories( $PathDir, $destination . '/' . $readdirectory );
                    continue;
                }
    ...
    static public function xmlNodeAttrsToArr($node) {
        ...
        foreach($node->attributes() as $a => $b) {
            $arr[$a] = utilsPrt::xmlAttrToStr($node, $a);
        }
        return $arr;
    }
    ...
    static public function getCurrentWPThemeDir() {
        static $themePath;
        if(empty($themePath)) {
            $themePath = get_theme_root(). DS. utilsPrt::getCurrentWPThemeCode(). DS; 
        }
        return $themePath;
    }
    /**
    ...
    static public function getExtModPath($plugName) {
        if(strpos($plugName, 'wp-content/themes') !== false) {  //for modules in theme dir
            return PRT_SITE_URL. str_replace(DS, '/', $plugName). '/';
        } else
    ...
    static public function isThisCommercialEdition() {
        ...
        foreach($commercialModules as $m) {
            if(!framePrt::_()->getModule($m)) 
                return false;
            if(!is_dir(framePrt::_()->getModule($m)->getModDir())) 
                return false;
        }
        return true;*/
        foreach(framePrt::_()->getModules() as $m) {
