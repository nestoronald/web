...
abstract class controllerPrt {
	...
	protected function _createModel($name = '') {
		if(empty($name)) $name = $this->getCode();
		$parentModule = framePrt::_()->getModule( $this->getCode() );
		$className = '';
		if(import($parentModule->getModDir(). 'models'. DS. $name. '.php')) {
			$className = toeGetClassNamePrt($name. 'Model');
		}
	...
	protected function _createView($name = '') {
		...
		if(import($parentModule->getModDir(). 'views'. DS. $name. '.php')) {
			$className = toeGetClassNamePrt($name. 'View');
		}
	 ...
	 * @example :
	 return array(
			S_METHODS => array(
				'save' => array(PRT_ADMIN),
				'remove' => array(PRT_ADMIN),
				'restore' => PRT_ADMIN,
			),
			S_USERLEVELS => array(
	 ...
	 * Can be used on of sub-array - PRT_METHODS or PRT_USERLEVELS
	 */
