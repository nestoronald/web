 ...
 */
if(!function_exists('dateToTimestampPrt')) {
    function dateToTimestampPrt($date) {
        if(empty($a))
            return false;
        $a = explode(PRT_DATE_DL, $date);
...
if(!function_exists('importClassPrt')) {
    function importClassPrt($class, $path = '') {
        if(!class_exists($class)) {
            if(!$path) {
				$classFile = $class;
				if(strpos(strtolower($classFile), PRT_CODE) !== false) {
					$classFile = preg_replace('/'. PRT_CODE. '$/i', '', $classFile);
				}
                $path = PRT_CLASSES_DIR. $classFile. '.php';
			}
            return import($path);
 ...
 */
if(!function_exists('toeGetClassNamePrt')) {
    function toeGetClassNamePrt($class) {
        $className = '';
		if(class_exists($class. strFirstUp(PRT_CODE))) 
			$className = $class. strFirstUp(PRT_CODE);
		else if(class_exists(PRT_CLASS_PREFIX. $class)) 
            $className = PRT_CLASS_PREFIX. $class;
		else
            $className = $class;
        return $className;
    } 
 ...
 */
if(!function_exists('toeCreateObjPrt')) {
    function toeCreateObjPrt($class, $params) {
        $className = toeGetClassNamePrt($class);
        $obj = NULL;
	...
	function prepareParams(&$d=array(), &$options = array()) {
		if (!empty($d['params'])) {
			...
			if (is_array($d['params'])) {
				$params = utilsPrt::jsonEncode($d['params']);
