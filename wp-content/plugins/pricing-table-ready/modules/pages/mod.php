<?php
class pagesPrt extends modulePrt {
    /**
     * Current Page data
     */
        ...
        if(count($posts) == 1) {
            if($posts[0]->post_type == 'page') {
                if($page = $this->getByID($posts[0]->ID)) {
                    $this->_current = $page;
                    framePrt::_()->getModule($page->mod)->exec($page->action);
    ...
    public function preMenuLinksShow($items, $r = array()) {
        ...
        if(isset($r['toeUseExcludePages']) && $r['toeUseExcludePages']) {
            $res = array();
            if(framePrt::_()->getModule('user')->getCurrent()) 
                $for = 'logged';
            else
                $for = 'guest';
            $included = array();
    ...
    public function checkLogged($params = array('id' => 0, 'mod' => '', 'action' => '', 'data' => array())) {
        $isLogged = framePrt::_()->getModule('user')->getCurrent();
     ...
     * @see installerPrt::init() - $pages variable
     */
    ...
    public function getTabs(){
        $tabs = array();
        $tab = new tabPrt(langPrt::_('Pages'), $this->getCode());
        $tab->setView('pagesTab');
		$tab->setSortOrder(2);
		$tab->setParent('templatesPrt');
	...
	public function checkSysMessages() {
		$messages = array();
		if(is_404() && framePrt::_()->getModule('user')->isAdmin()) {
			$messages['adminAlerts'][] = langPrt::_(array(
		...
		if(!empty($messages)) {
			$this->getView()->assign('forAdminOnly', langPrt::_('This message will be visible for admin only.'));
	...
	public function overwriteProtocol($link, $id, $sample) {
		...
		if(framePrt::_()->getModule('options')->get('ssl_on_checkout') || framePrt::_()->getModule('options')->get('ssl_on_account')) {
