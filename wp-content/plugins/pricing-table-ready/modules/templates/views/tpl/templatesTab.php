...
// <!--
    ...
    function toeUpdateTemplate(newValue) {
        jQuery(this).sendForm({
            data: {
                reqType: 'ajax', action: 'putOption', page: 'optionsPrt', code: 'default_theme', value: newValue
            },
            msgElID: 'toeTplMsg',
            onSuccess: function(res) {
                if(!res.error) {
                    jQuery('#toeTplMsg').html('<?php langPrt::_e('Template Updated. Reload page to see results.')?>');
...
?>
                <td width="<?php echo ceil(100/$tplPerRow)?>%">
                    <div class="toeTplOption">
                        ...
                        <a href="<?php echo $code?>" class="toeTplActivateLink"><?php langPrt::_e('Activate')?></a>
                        <a href="<?php echo $code?>" class="toeTplDeactivateLink"><?php langPrt::_e('Deactivate')?></a><br /><br />
